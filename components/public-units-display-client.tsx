...
import { PublicUnit } from '@/lib/public-units-service';

interface PublicUnitsDisplayClientProps {
  city?: string; // Optional filter by city
}

export function PublicUnitsDisplayClient({ city }: PublicUnitsDisplayClientProps) {
  ...
  useEffect(() => {
    const fetchUnits = async () => {
      try {
        ...
        const allUnits: PublicUnit[] = await response.json();
        ...
        // Limit to 6 units for display
        setUnits(filteredUnits.slice(0, 6));
      } catch (err) {
        ...
        >
          <div className="aspect-[4/3] overflow-hidden bg-muted rounded-lg relative">
            ...
            {unit.status === 'out-of-service' && (
              <Badge className="absolute top-4 right-4 bg-yellow-600 hover:bg-yellow-600" variant="secondary">
                Maintenance
              </Badge>
            )}
          </div>
