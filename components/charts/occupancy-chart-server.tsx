...
export async function OccupancyChartServer({ hotelId }: { hotelId: string }) {
  try {
    ...
    if (unitsError || !totalUnits || totalUnits === 0) {
      const defaultData: OccupancyData[] = [
        { day: "السبت", occupancy: 0 },
        { day: "الأحد", occupancy: 0 },
        { day: "الإثنين", occupancy: 0 },
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const { count: occupiedUnits, error: occupancyError } = await supabase
        .from('bookings')
        .select('*', { count: 'exact', head: true })
        .eq('hotel_id', hotelId)
      ...
      if (occupancyError) {
        console.error(`Error getting occupancy for date ${date}:`, occupancyError);
        occupancyData.push({
          day: dayName,
          occupancy: 0
        });
    ...
    console.error("Error in OccupancyChartServer:", error);
    const defaultData: OccupancyData[] = [
      { day: "السبت", occupancy: 0 },
      { day: "الأحد", occupancy: 0 },
      { day: "الإثنين", occupancy: 0 },
